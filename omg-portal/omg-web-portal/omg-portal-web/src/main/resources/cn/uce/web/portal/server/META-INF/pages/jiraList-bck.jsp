...
<html>
	...
	<body class="easyui-layout">
		    ...
		    $(document).ready(function(){
                ...
                var columns = [[
                    {field:'key',title:'编号',align:'center',width: '6%',formatter: function(value,row,index){
						return '<a style="color:#6a54ba" href="javascript:void(0);" onclick="viewDetail(\'' + row.key + '\')">'+ row.key +'</a>';
					}},
                    ...
                    {field:'status',title:'状态',align:'center',width: '8%',formatter: function(value){
                    	if (value == '待受理') {
                    ...
                    {field:'score',title:'处理结果确认',align:'center',width: '5%',formatter: function(value,row,index) {
						
						if (row.status == "处理完成") {
							return '<img alt="" onclick="openScore(\'' + row.key+ '\',2, \'处理完成\')" src="../scripts/portal/images/face_y.png"></img><img style="margin-left:15px;" alt="" onclick="openScore(\'' + row.key+ '\',1,\'处理完成\')" src="../scripts/portal/images/flower_y.png"></img>';
						}
						var thirtyCheck = dataCompareThirty(row.endTime);
						if (!thirtyCheck && row.status == "关闭") {
							return '<img alt="" onclick="openScore(\'' + row.key+ '\',2, \'关闭\')" src="../scripts/portal/images/face_y.png"></img><img style="margin-left:15px;" alt="" onclick="openScore(\'' + row.key+ '\',1, \'关闭\')" src="../scripts/portal/images/flower_y.png"></img>';
						}
						return '<img alt="" src="../scripts/portal/images/face_n.png"></img><img style="margin-left:15px;" src="../scripts/portal/images/flower_n.png"></img>';
					}}
                ]];
		       var dataGridParams = {
					url : '${pageContext.request.contextPath}/jira/getIssueList.do',
					pageSize : 20,
					...
					onLoadSuccess : function(data) {
						...
						/* waitStatus = data.otherParam.waitStatus;//待处理
						processStatus = data.otherParam.processStatus;//处理中
						confirmStatus = data.otherParam.confirmStatus;//待确认
						overStatus = data.otherParam.overStatus;//已完成
						 */
						confirmStatus = data.otherParam.confirmStatus;//待确认
						...
						if (statusParam == 2) {//待处理
							
							$('#waitStatusA').css('color','red');
						} else if (statusParam == 3) {//处理中
							
							$('#processStatusA').css('color','red');
						} else if (statusParam == 4) {//待确认
						
							$('#confirmStatusA').css('color','red');
						} else if (statusParam == 5) {//已完成
						...
						/* $("#allStatus").text(waitStatus + processStatus + confirmStatus + overStatus);
						$("#waitStatus").text(waitStatus);
						$("#processStatus").text(processStatus);
						$("#confirmStatus").text(confirmStatus);
						$("#overStatus").text(overStatus); */
					}
				}
				...
				/**
				uceDictCombobox("issuetype","JIRA_TYPE",{headerValue:false});
				var data = $('#issuetype').combobox('getData');
				if(null != data && data.length > 0) {
					$("#issuetype").combobox('select',data[0].typeCode);
				}
				**/
		...
		function resetQuery(){
		  ...
		  //$("#issuetype").combobox('select', '全部');
		}

		function viewDetail(key) {
			var title = "工单详情(" + key + ")" ;
			
			var url = localHref + "${pageContext.request.contextPath}/jira/jiraDetail.do";
			
			openTabCrossDomain(null,{issueId:key},{title:title,systemCode:"PORTAL",url:url});
		}
		function initInfo() {
		...
		function ITServiceReport() {
			var url = localHref + "${pageContext.request.contextPath}/jira/createIssue.do";
			...
			$("#saveJiraIssueForm2").form('clear');
			initInfo();
			$("#issueTypeId1").next().hide();
		    $('#issueTypeId1').textbox('setValue','10306');
		    $("#issueTypeId2").next().hide();
		    $('#issueTypeId2').textbox('setValue','10308');
		    $('#description1').textbox('setValue','');
			$('#description2').textbox('setValue','');
		    $('#img_preview1').empty();
						...
						if(nVal){
							...
							fileArr.forEach((item,index) => {
							    if(item){
									...
									imageNum++;
							        //读取本地图片，以base64编码输出
						            $('#img_preview1').append('<div class="uploaddiv" id="uploaddiv_'+imageNum+'">'+
						            		'<div class="imgList"><img src="'+this.result+'" id="uploadimg_'+imageNum+'" alt="'+item.name+'" title="'+item.name+'"><img class="delect" onclick="deleteImage(\'uploaddiv_'+ imageNum +'\','+imageNum+')" src="../scripts/portal/images/delect.png">'+
			...
			$('#file2').filebox({
				...
				onChange:function(nVal,oVal){
					if(nVal){
						let fileObjs=$("input[name=file]")[1].files;
						if(!fileCheck(fileObjs)){
							return;
						}
						var fileArr = Array.from(fileObjs);
						...
						fileArr.forEach((item,index) => {
						    if(item){
								...
								imageNum++;
						        //读取本地图片，以base64编码输出
					            $('#img_preview2').append('<div class="uploaddiv" id="uploaddiv_'+imageNum+'">'+
						            		'<div class="imgList"><img src="'+this.result+'" id="uploadimg_'+imageNum+'" alt="'+item.name+'" title="'+item.name+'"><img class="delect" onclick="deleteImage(\'uploaddiv_'+ imageNum +'\','+imageNum+')" src="../scripts/portal/images/delect.png">'+
						            		'<p style="text-align:center;">'+item.name+'</div>' +
						            		'<input class="easyui-textbox" id="imgBase'+imageNum+'" name="imageBase" style="width:180px"/>'+
						            		'<input class="easyui-textbox" id="imgBaseName'+imageNum+'" name="imageBaseName" style="width:180px"/></div>');
			...
			$('#jiraAddInfo').dialog('options').title = 'IT服务台(IT热线021-31290988)';
			$('#jiraAddInfo').dialog({modal:true}).dialog('open');
			$("#tt").tabs("select", 0);
			var description1 = DictDatas['PROMPT'][0];
			if (description1 == undefined) {
				description1 = "示范：异常系统：OA 异常描述：密码遗忘登陆不了OA，点击忘记密码没反应，请帮我解决。";
			} else {
				description1 = DictDatas['PROMPT'][0].typeName;
			}
			var description2 = DictDatas['PROMPT'][1];
		...
		function saveJiraIssue(btn) {
			...
			$("#" + saveJiraIssueForm).form('submit',{
	  			...
	  			task: function(result){
					...
					/* $.messager.alert('创建问题成功','<img src="../scripts/portal/images/face_small.png"></img><lable style="font-size:15px;margin-left:10px;">提交成功，请耐心等待工程师为您处理！</lable>','infoSunnyIcon',function(){});
					setTimeout(function(){$(".messager-body").window('close');},3000); */
		...
		function addScore(btn) {
			$(btn).linkbutton('disable');
			var url = "${pageContext.request.contextPath}/jira/addComment.do";
			if (processValue == "关闭") {
				url = "${pageContext.request.contextPath}/jira/insertComment.do";
			}
			$("#saveYJiraScoreForm").form('submit',{
	  			url : url,
	  			task: function(result){
					closeDialog('jiraAddYScore');
					/* $.messager.alert('反馈问题成功','<img src="../scripts/portal/images/face_small.png"></img><lable style="font-size:15px;margin-left:10px;">感谢您的反馈！</lable>','infoSunnyIcon',function(){});
					setTimeout(function(){$(".messager-body").window('close');},3000); */
					reloadDatagrid('dgOperatorLog');
	  			},
	  			finalTask:function(){
					...
					<div id="tlbOperatorLog" style="padding:5px;height:auto">
						<form action="#" id="formfindOperatorLog">
							...
							<div style="line-height:20px;font-size:14px;font-family:微软雅黑;margin-left:15px;">
								...
								<span>|</span><a id="waitStatusA" onclick="findOperatorLog(2)" style="margin-left:15px;margin-right:10px;">待处理<!-- (<span id="waitStatus"></span>) --></a>
								<span>|</span><a id="processStatusA" onclick="findOperatorLog(3)" style="margin-left:15px;margin-right:10px;">处理中<!-- (<span id="processStatus"></span>) --></a>
								<span>|</span><a id="confirmStatusA" onclick="findOperatorLog(4)" style="margin-left:15px;margin-right:10px;">待确认(<span id="confirmStatus"></span>)</a>
								<span>|</span><a id="overStatusA" onclick="findOperatorLog(5)" style="margin-left:15px;margin-right:10px;">已完成<!-- (<span id="overStatus"></span>) --></a>
								<span>|</span>
