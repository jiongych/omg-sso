...
import uce.sso.sdk.stream.converters.reflection.FieldDictionary;
import uce.sso.sdk.stream.converters.reflection.ObjectAccessException;
import uce.sso.sdk.stream.converters.reflection.PureJavaReflectionProvider;
...
import uce.sso.sdk.stream.core.util.PresortedMap;
import uce.sso.sdk.stream.core.util.PresortedSet;
...
/**
 * JVM implements Caching  
 * @Description: JVM implements Caching  
 * @author automatic 
...
public class JVM implements Caching {
    ...
    static {
        ...
        if (canUseSunUnsafeReflectionProvider()) {
            Class cls = loadClassForName("uce.sso.sdk.stream.converters.reflection.SunUnsafeReflectionProvider");
            if (cls != null) {
                try {
                    ...
                    if (cls == null) {
                        cls = loadClassForName("uce.sso.sdk.stream.converters.reflection.SunLimitedUnsafeReflectionProvider");
                    }
                    type = cls;
                } catch (ObjectAccessException e) {
                }
