...
public class IPSeeker {
	protected static final Log LOG = LogFactory.getLog(IPSeeker.class);
	 ...
	 */
	public List<IPEntry> getIPEntriesDebug(String strIp) {
		if (strIp == null) {
			strIp = "";
		}
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (ipLoc.getCountry().indexOf(strIp) != -1 || ipLoc.getArea().indexOf(strIp) != -1) {
	...
	private IPSeeker(String path) {
		ipCache = new Hashtable<String, IPSeeker.IPLocation>();
		loc = new IPLocation();
		b4 = new byte[4];
		b3 = new byte[3];
		try {
			...
			LOG.error(IPSeeker.class.getResource("QQWry.dat").toString());
			LOG.error(ip_file);
